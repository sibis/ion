...
func randSeq(n int) proto.RID {
	...
	return proto.RID(stringb)
}

// FindMyLuckyRoom helps finding my lucky room
func FindMyLuckyRoom(id proto.RID) proto.RID {
	for _, room := range rooms {
		if len(room.ID()) > 15 {
			people := len(room.GetPeers())
			if people < 2 {
				return room.ID()
			}
		}
